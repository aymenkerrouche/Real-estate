// ignore_for_file: avoid_unnecessary_containers, prefer_const_constructors, prefer_const_literals_to_create_immutables, unused_import, avoid_print, sized_box_for_whitespace
...
class _AccountPageState extends State<AccountPage> {
  ...
  void _startTimer() {
    Timer(const Duration(seconds: 1), () {
        ...
        logout();
        Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context)=>XDLanding()), (route) => false);
    });
  }
  ...
  getHeader(){
    return
      Row(
        ...
        children: [
          IconBox(
            child: SvgPicture.asset('assets/icons/setting.svg', color: primary,), radius: 50),
        ],
      );
  }
    ...
    SingleChildScrollView(
      ...
      child: Column(
        children: [
              ...
              Stack(
                children: [
                  Container(
                    ...
                    child: ClipOval(
                      child: user!.image == 'user.png' ? CustomImage(profile["image"]!,bgColor: appBarColor,) :
                        CustomImage(imageURL,bgColor: appBarColor,)
                    ),
                  ),
                  Positioned(
                    bottom: -13,right: -13,
                    child: Container(
                      ...
                      child: IconButton(
                        ...
                        icon: Icon(Icons.add_a_photo_outlined,color: darker,),
                        onPressed: () => pickPic(),
                      ),
                    ),
                  ),
          ...
          loading == false ? Center(child: CircularProgressIndicator(color: mainColor,))
          :Container(
            padding: const EdgeInsets.only(left: 15, right: 15),
            decoration: BoxDecoration(
              borderRadius: BorderRadius.circular(5),
    ...
    return showModalBottomSheet(
      ...
      builder: (context) =>
        Column(
          ...
          children: [
            Container(margin: EdgeInsets.only(top: 3),width: size.width * 0.1,height: 4,decoration: BoxDecoration(borderRadius: BorderRadius.circular(50),color: mainColor),),
            ListTile(
              leading: Icon(Icons.camera_alt,color: primary,),
              title: Text('Camera',style: TextStyle(color: primary)),
              onTap: ()=>pickImage(ImageSource.camera)),
            Divider(height: 15,color: cardColor,),
            ListTile(
